...
# CSV 파일 경로
csv_path = "/data/ephemeral/home/data/train.csv"
...
llm_label = OllamaLLM(model="gemma2:27b")
...
for i in range(0, len(df), batch_size):
    ...
    try:
        # 첫 번째 체인 호출
        response_reasoning = chain_reasoning.invoke({"batch_size": len(batch_texts), "input": input_text})
    ...
    # 텍스트2: 0
    # ...
    responses = [line.strip() for line in response_label.strip().split('\n') if line.strip()]
...
# 14. 유효하지 않은 응답(None)을 0으로 대체
df['is_noise'].fillna(0, inplace=True)

# 15. 새로운 CSV 파일로 저장
output_csv_path = "/data/ephemeral/home/data/is_noise_train.csv"
