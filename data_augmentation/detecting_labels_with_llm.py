...
5. 마크다운 bold를 사용하지 않고 최종 출력의 형식을 엄격하게 지켜서 작성해 주세요.
    ...
    try:
        ...
        for resp in responses:
            ...
            if match:
                ...
                label_category_list.append((idx, category, reasoning))
            else:
                # 형식이 맞지 않으면 기본값 설정
                idx = resp.split(":")[0] if ":" in resp else "0"
                category = "Uncategorized"
                label_category_list.append((idx, category, "No reasoning provided"))
...
df_labels = pd.DataFrame(
    label_category_list, columns=["label", "category", "reasoning"]
)
# breakpoint()
df_labels["category"] = (
...
reasoning_per_label = df_labels.drop_duplicates(subset=["label"])[
    ["label", "reasoning"]
]
counts_with_reasoning = counts_filtered.merge(
    reasoning_per_label, on="label", how="left"
)
# 결과 출력
...
print("\n### 라벨별 가장 많이 나온 카테고리 ###")
print(most_common)
df_labels.to_csv("df_labels_with_reasoning.csv", index=False)
# CSV 파일로 저장 (필요에 따라)
counts_with_reasoning.to_csv("label_category_counts_with_reasoning.csv", index=False)
