...
from utils_seona import label_group_count, filter_similar_texts, call_ollama_with_retry
...
label_meanings = {row['label']: row['meaning'] for _, row in label_meanings_df.iterrows()} # 각 라벨의 의미를 딕셔너리 형태로 변환
label_meanings_text = "\n".join([f"{label}: {meaning}" for label, meaning in label_meanings.items()]) # 모든 라벨 의미를 한 번에 프롬프트에 포함하여 작성

## 현재 라벨별 데이터 수정
data_df = pd.read_csv('../data/noise_label_cleaned_train.csv') # 노이즈, 라벨 에러 제거된 train 데이터
data_df = shuffle(data_df).reset_index(drop=True) # 데이터 shuffle
current_counts = label_group_count(data_df)
target_count = max(current_counts.values()) # 목표 데이터 수 설정 (최대 데이터 수 기준으로 맞추기)
...
def augment_data(data_df, label_meanings, target_count, similarity_threshold=0.7):
    ...
    for label, meaning in label_meanings.items():
        ...
        for existing_text in existing_texts:
            ...
            print(f"증강 대상 텍스트 (라벨 {label}): {existing_text}")  # 증강 대상 텍스트를 출력
            prompt = generate_prompt(label, label_meanings_text, meaning, [existing_text])
            ...
            for text in generated_texts:
                ...
                if current_count >= target_count:
                    break  # 목표 개수 도달 시 종료
            '''
            cleaned_text = response.replace("뉴스 기사 제목:", "").strip()  # 응답에서 "뉴스 기사 제목:" 제거
            if cleaned_text and cleaned_text not in generated_text_set:
                new_id = f"{label}_aug_{len(augmented_texts):05d}"  # 고유 ID
                augmented_texts.append({'ID': new_id, 'text': cleaned_text, 'target': label})
                generated_text_set.add(cleaned_text)  # 추가된 텍스트를 집합에 저장
                print(f"증강된 텍스트 (라벨 {label}): {cleaned_text}")  # 추가된 텍스트 확인용


    # 중복 및 유사 텍스트 제거
    augmented_df = pd.DataFrame(augmented_texts, columns=['ID', 'text', 'target'])  # 열 이름을 명시적으로 지정
    # 중복 및 유사 텍스트 제거 후 DataFrame으로 변환
    filtered_texts = filter_similar_texts(augmented_df['text'].tolist(), label, similarity_threshold)

    # 필터링한 텍스트로 DataFrame 생성
    filtered_df = pd.DataFrame({'ID': [f"{label}_aug_{i:05d}" for i in range(len(filtered_texts))],
                                'text': filtered_texts,
                                'target': label})
